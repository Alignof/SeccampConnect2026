  ...
  outputs =
    ...
    // flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        # Capture the environment generated by Nix hooks (e.g., IDF_PYTHON_ENV_PATH)
        # This guarantees the Docker container has the exact same environment as devShell
        esp-env =
          pkgs.runCommand "esp-env"
            {
              buildInputs = devDeps;
            }
            ''
              # Running the standard stdenv setup evaluates all package hooks
              source ${pkgs.stdenv}/setup

              mkdir -p $out/etc/profile.d
              # Dump the evaluated Nix store paths and variables to a script
              # The first $PATH is evaluated at build time, \$PATH appends the runtime container path
              cat <<EOF > $out/etc/profile.d/nix-esp.sh
              if [ -z "\$NIX_ESP_ENV_LOADED" ]; then
      ...
      in
      {
        ...
        packages.dockerImage = pkgs.dockerTools.buildLayeredImage {
          ...
          contents = devDeps ++ [
            ...
            # Inject the generated environment script into the image
            esp-env
          ];
          ...
          fakeRootCommands = ''
            ...
            # Setup profile and bashrc to auto-load the Nix environment automatically
            # This ensures 'docker exec' and interactive shells work flawlessly
            echo "source /etc/profile.d/nix-esp.sh" > ./etc/profile
            echo "source /etc/profile.d/nix-esp.sh" > ./etc/bashrc
            mkdir -p ./root
          ...
          config = {
            Cmd = [ "/bin/bash" ];
            Env = [
              # BASH_ENV and ENV ensure non-interactive shells also source the script
              "BASH_ENV=/etc/profile.d/nix-esp.sh"
              "ENV=/etc/profile.d/nix-esp.sh"
